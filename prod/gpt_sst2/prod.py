    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                # 更新回数のカウント
                state['sgd_count'] += p.numel()  # パラメータの要素数分カウント
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # 更新回数のカウント
                state['adam_count'] += p.numel()  # パラメータの要素数分カウント
...
# データセットの準備
def prepare_dataset(tokenizer, dataset_name="sst2", max_seq_length=128, batch_size=16):
    """テキスト分類のデータセットを準備"""
    # データセットのロード（Hugging Face datasets）
    ...
    if dataset_name == "sst2":
        ...
        # 他のデータセットでの対応
        train_texts = dataset["train"]["text"] if "text" in dataset["train"].features else dataset["train"]["sentence"]
        train_labels = dataset["train"]["label"]
        
        test_texts = dataset["validation"]["text"] if "text" in dataset["validation"].features else dataset["validation"]["sentence"]
        test_labels = dataset["validation"]["label"]
    
    # カスタムデータセットの作成
    train_dataset = TextClassificationDataset(
        train_texts, train_labels, tokenizer, max_length=max_seq_length
    )
    
    test_dataset = TextClassificationDataset(
        test_texts, test_labels, tokenizer, max_length=max_seq_length
    )
...
# 訓練関数
def train(model, optimizer, train_loader, test_loader, scheduler, criterion, epoch, device, 
...
# オプティマイザの比較実験
def run_optimizer_comparison(seed=42, num_epochs=3, batch_size=16, max_seq_length=128, log_interval=100):
    set_seed(seed)
    
    # GPT-2モデルとトークナイザーの準備
    model_name = "gpt2"  # より小さいGPT-2モデルを使用
    tokenizer = GPT2Tokenizer.from_pretrained(model_name)
    
    # GPT-2はCLSトークンがないため、終了トークンをパディングトークンとして設定
    tokenizer.pad_token = tokenizer.eos_token
    
    # データローダーの準備
    train_loader, test_loader = prepare_dataset(
        tokenizer, dataset_name="sst2", max_seq_length=max_seq_length, batch_size=batch_size
    )
    
    # 損失関数
    criterion = nn.CrossEntropyLoss()
    
    # オプティマイザの設定（カスタム実装を使用）
    optimizers = {
        'SGD_Momentum': lambda m: SGDMomentum(
            m.parameters(), 
            lr=5e-5,
            momentum=0.9, 
            weight_decay=0.01
    ...
    for name, opt_fn in optimizers.items():
        ...
        model.config.pad_token_id = model.config.eos_token_id  # パディングトークンの設定
        ...
        warmup_steps = int(0.1 * total_steps)  # 全ステップの10%をウォームアップに
        ...
        for epoch in range(num_epochs):
            ...
            train_loss, train_acc = train(
                model, optimizer, train_loader, test_loader, scheduler, criterion, epoch, device,
        ...
        # モデルの保存
        model_save_path = f'gpt2_{name}_model.pt'
        torch.save({
...
def plot_results(results, num_epochs):
    ...
    plt.tight_layout()
    plt.savefig('gpt2_optimizer_comparison_steps.png')
    plt.show()
...
# メイン実行関数（収束分析の追加）
def main():
    print("Starting Optimizer Comparison Experiment on GPT-2 with Text Classification")
    print("Comparing Three Optimizers: SGD with Momentum, Adam, and EAGLE")
    print("All optimizers are implemented from scratch for fairness.")
    
    # 実験の実行
    num_epochs = 7
    batch_size = 16  # メモリ制約に合わせて調整
    max_seq_length = 128  # 最大シーケンス長
    log_interval = 100  # 100バッチごとにメトリクスを記録
    ...
    print("-" * 80)
    print(f"{'Optimizer':<25} {'Time per epoch':>15} {'Time per 100 steps':>20}")
    print("-" * 80)
    
    for name, res in results.items():
        total_time = res['time'][-1]
        total_steps = res['steps'][-1]
        time_per_100_steps = (total_time / total_steps) * 100 if total_steps > 0 else 0
        print(f"{name:<25} {total_time:>14.2f}s {time_per_100_steps:>19.2f}s")
